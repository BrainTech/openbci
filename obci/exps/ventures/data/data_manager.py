...
CALIBRATION_RESULTS_2 = 'calibration_results_2.csv'
...
def calibration_set(user_id, up, right, down, left, file_name):
    ...
    data.set_value(data[data['ID']==user_id].index[0], '{}_time'.format(number), current_time_to_string())
    data.set_value(data[data['ID']==user_id].index[0], '{}_file_name'.format(number), file_name.__repr__())
    data.to_csv(get_database_file_name(CALIBRATION_RESULTS),
                cols=col_mame_to_save_get(data, ['up','down','left','right','time', 'file_name'])) 

def calibration_2_set(user_id, up, right, down, left, file_name):
    data = pd.read_csv(get_database_file_name(CALIBRATION_RESULTS_2), index_col=0, dtype='str')
    ...
    data.to_csv(get_database_file_name(CALIBRATION_RESULTS_2),
                cols=col_mame_to_save_get(data, ['up','down','left','right','time', 'file_name'])) 
...
def calibration_2_get_last(user_id):
    data = pd.read_csv(get_database_file_name(CALIBRATION_RESULTS_2), index_col=0,  dtype='str')
    if user_id in data['ID'].values:
        number = data[data['ID']==user_id]['number'].values[0]
        up = data[data['ID']==user_id]['{}_up'.format(number)].values[0]
...
def wii_current_level_set(user_id, level_up, level_down, level_left, level_right):
    ...
    data.to_csv(get_database_file_name(GAME_WII_RESULTS), 
                                       cols=col_mame_to_save_get(data, ['level_up', 'level_down', 'level_left', 'level_right','time']))     
