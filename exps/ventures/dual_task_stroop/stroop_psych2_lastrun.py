...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.80.00), Tue 20 Jan 2015 10:43:23 PM CET
...
filename = '~/stroop' + os.path.sep + '%s_%s' %(expInfo['participant'], expInfo['date'])
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/home/ania/stroop/stroop_psych2.psyexp',
...
text_7 = visual.TextStim(win=win, ori=0, name='text_7',
    ...
    color='white', colorSpace='rgb', opacity=1,
    depth=-1.0)
tagger_4 = contrib.obci.TagOnFlip(
        window=win,
        tagName="timestamp", tagDescription={},        doSignal=False,
        sendTags=thisExp.sendTags, saveTags=thisExp.saveTags)
tagger_4.setTagDescription({'value':time.time()})
tagger_4.setTagName('timestamp')
...
trenujemy = data.TrialHandler(nReps=1, method='no-repeat random', 
    extraInfo=expInfo, originPath=u'/home/ania/stroop/stroop_psych2.psyexp',
...
instrukcjaComponents.append(tagger_4)
    ...
    if key_resp_5.status == STARTED:
        ...
        if len(theseKeys) > 0:  # at least one key was pressed
            if key_resp_5.keys == []:  # then this was the first keypress
                ...
                # a response ends the routine
                continueRoutine = False
    if t >= 0.0 and tagger_4.status == NOT_STARTED:
        # keep track of start time/frame for later
        tagger_4.tStart = t  # underestimates by a little under one frame
        tagger_4.frameNStart = frameN  # exact frame index
        tagger_4.scheduleStart()
        tagger_4.status = STARTED
    elif tagger_4.status == STARTED and frameN >= (tagger_4.frameNStart + 1):
        tagger_4.scheduleStop()
        tagger_4.status = FINISHED
