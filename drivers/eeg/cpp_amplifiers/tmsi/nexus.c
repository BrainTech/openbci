...
float tms_get_float(uint8_t *msg, int32_t *sb) {
	...
	/* construct float */
	a = sign * mant * pow(2.0, expo - 23.0 - 127);
...
char *tms_get_string(uint8_t *msg, int32_t n, int32_t start) {
	...
	if (i > n) {
		fprintf(stderr,
				"# Error: tms_get_string: index 0x%04X out of range 0x%04X\n",
				i, n);
	}
	...
	if (i + size > n || size <= 0) {
		fprintf(
				stderr,
				"# Error: tms_get_string: index 0x%04X out of range 0x%04X (length:%d)\n",
				i + size, n, size);
	} else {
 ...
 */
int32_t tms_write_log_msg(uint8_t *msg, int32_t n, const char *comment) {
	...
	while (!feof(fpl)) {
		...
		if (ni != 5) {
			if (sec == 0) {
				fprintf(
						stderr,
						"# Error: tms_read_log_msg : wrong argument count %d on line %d. Skip it\n",
						ni, lc);
			}
			sec++;
			continue;
		...
		if (ba >= n) {
			fprintf(
					stderr,
					"# Error: tms_read_log_msg : size array 'msg' %d too small %d\n",
					n, nr);
		} else if (en == nr) {
			msg[ba + 1] = d1;
			msg[ba] = d0;
...
int32_t tms_get_cfg(uint8_t *msg, int32_t *i, tms_config_t *cfg) {
	...
	for (j = 0; j < 64; j++) {
		...
		cfg->storageType[j].overflow = (0xFFFFFF80)
				<< (8 * (cfg->storageType[j].delta - 1)); /**< overflow */
	}
	for (j = 0; j < 12; j++) {
	...
	for (j = 0; j < cfg->nrOfChannels; j++) {
		if ((cfg->storageType[j].delta > 0)
				&& (cfg->storageType[j].deci < cfg->mindecimation)) {
	...
	for (j = 0; j < cfg->nrOfChannels; j++) {
		if (cfg->storageType[j].delta > 0) {
			cfg->storageType[j].period = (cfg->storageType[j].deci + 1)
					/ (1 << cfg->mindecimation);
		}
	}
	...
	for (i = 0; i < 64; i++) {
		if (cfg->storageType[i].delta != 0) {
			nc += fprintf(fp, "s %2d %5d %5d %5d %5d ;  %1c %6d\n", i,
					cfg->storageType[i].ref, cfg->storageType[i].deci,
					cfg->storageType[i].delta, cfg->storageType[i].shift,
					'A' + i, cfg->storageType[i].period);
		}
	}
	if (prt_info) {
...
int32_t tms_rd_cfg(FILE *fp, tms_config_t *cfg) {
	...
	while (!feof(fp) && (go_on == 1)) {
		...
		case 'f':
			...
			break;
		case 's':
			sscanf(line, "s %d %d %d %d %d ;", &nr, &ref, &decim, &delta,
					&shift);
			endptr = NULL;
			if ((nr >= 0) && (nr < 64)) {
		...
		if (endptr != NULL) {
			fprintf(stderr,
					"# Warning: line %d has an configuration error!!!\n", lc);
		}
	}
	return (lc);
}
 ...
 * @return number of bytes parsed
 */
int32_t tms_get_measurement_hdr(uint8_t *msg, int32_t *i,
		tms_measurement_hdr_t *hdr) {
	...
	if (err != 0) {
		fprintf(
				stderr,
				"# Warning: end time incorrect, unexpected end of recording!!!\n");
	}
...
int32_t tms_prt_ack(FILE *fp, tms_acknowledge_t *ack) {
	...
	nc += fprintf(fp, "# Ack: desc %04X err %04X", ack->descriptor,
			ack->errorcode);
	...
	case 0x15:
		nc += fprintf(fp,
				" nr of words to read from flash memory out of range");
		break;
	case 0x16:
...
int32_t tms_prt_frontendinfo(FILE *fp, tms_frontendinfo_t *fei, int nr, int hdr) {
	...
	if (hdr) {
		...
		nc +=
				fprintf(
						fp,
						"# %3s %3s %2s %4s %9s %4s %4s %4s %4s %4s %4s %4s %4s %4s %4s\n",
						"uch", "css", "md", "mxfs", "serialnr", "nEXG", "nAUX",
						"hwv", "swv", "cmds", "snds", "nc", "bfs", "pw",
						"hwck");
	}
	nc +=
			fprintf(
					fp,
					" %4d %3d %02X %4d %9d %4d %4d %04X %04X %4d %4d %4d %4d %04X %04X\n",
					fei->nrofuserchannels, fei->currentsampleratesetting,
					fei->mode, fei->maxRS232, fei->serialnumber, fei->nrEXG,
					fei->nrAUX, fei->hwversion, fei->swversion, fei->cmdbufsize,
					fei->sendbufsize, fei->nrofswchannels, fei->basesamplerate,
					fei->power, fei->hardwarecheck);
	return (nc);
}
 ...
 * @return 0 on success, -1 on failure
 */
int32_t tms_get_type_desc(uint8_t *msg, int32_t n, int32_t start,
		tms_type_desc_t *td) {
 ...
 */
int32_t tms_get_input_device(uint8_t *msg, int32_t n, int32_t start,
		tms_input_device_t *inpdev) {
	...
	inpdev->Channel = (tms_channel_desc_t *) calloc(inpdev->NrOfChannels,
			sizeof(tms_channel_desc_t));
	...
	for (j = 0; j < inpdev->NrOfChannels; j++) {
		...
		if (nb % 8 != 0) {
			fprintf(stderr,
					"# Warning: tms_get_input_device: channel %d has %d bits\n",
					j, nb);
		}
		tnb += nb;
	}
	if (tnb % 16 != 0) {
		fprintf(
				stderr,
				"# Warning: tms_get_input_device: total bits count %d %% 16 !=0\n",
				tnb);
	}
...
int32_t tms_prt_type_desc(FILE *fp, tms_type_desc_t *td, int nr, int hdr) {
	...
	if (hdr) {
		nc += fprintf(fp, " %6s %4s %4s %4s %4s %2s %3s %9s %9s %3s %4s %3s\n",
				"size", "type", "typd", "sty", "styd", "sg", "bit", "a", "b",
				"uid", "uidd", "exp");
	}
	if (td == NULL) {
		return (nc);
	...
	nc += fprintf(fp, " %2s %3d %9e %9e %3d", (td->Format & 0x0100 ? "S" : "U"),
			(td->Format & 0xFF), td->a, td->b, td->UnitId);
	/* UnitId description */
	switch (td->UnitId) {
	case 0:
 ...
 * @return number of printed characters.
 */
int32_t tms_prt_channel_data(FILE *fp, tms_input_device_t *dev,
		tms_channel_data_t *chd, int32_t md) {
...
int32_t get_channel_data_int(uint8_t *msg, int *s, int b) {
	...
	int i = *s;
	if (b == 4 && !ip_amplifier) {
		ans = (msg[i + 1] << 24) | (msg[i] << 16) | (msg[i + 3] << 8)
				| (msg[i + 2]);
		(*s) += b;
	} else
...
int32_t tms_get_data(uint8_t *msg, int32_t n, tms_input_device_t *dev,
	...
	for (j = 0; j < dev->NrOfChannels; j++) {
		...
		if (dev->Channel[j].Type.Format & 0x0100) {
			chd[j].data[0].isample = (chd[j].data[0].isample << (32 - 8 * nbps))
					>> (32 - 8 * nbps);
		}
	...
	if (type == TMSVLDELTADATA) {
		...
		while ((cnt < totns) && (bip < 8 * n - 16)) {
			...
			fprintf(stderr, "find channel not in overflow and needs this sample");
			/* find channel not in overflow and needs this sample */
			while ((chd[j].data[0].flag & 0x01) || (chd[j].rs >= chd[j].ns)
					|| ((pc % srp[j]) != 0)) {
				/* next channel nr */
				j++;
	...
	for (j = 0; j < dev->NrOfChannels; j++) {
		...
		for (i = 0; i < chd[j].ns; i++) {
			chd[j].data[i].sample = (dev->Channel[j].Type.a)
					* chd[j].data[i].isample + dev->Channel[j].Type.b;
		}
 ...
 * @return number of characters printed.
 */
int32_t tms_prt_samples(FILE *fp, tms_channel_data_t *chd, int32_t cs,
		int32_t md) {
	int32_t nc = 0; /**< number of characters printed */
	...
	if (mns > maxns) {
		...
		prt = (tms_data_t *) calloc(maxns * tms_get_number_of_channels(),
				sizeof(tms_data_t));
	}
	...
	for (i = 0; i < mns; i++) {
		nc += fprintf(fp, " %9.4f %8d", (chd[jm].sc + i) * chd[jm].td,
				chd[jm].sc + i);
		for (j = 0; j < tms_get_number_of_channels(); j++) {
			if (cs & (1 << j)) {
				idx = maxns * j + i;
 ...
 */
int32_t tms_get_vldelta_info(uint8_t *msg, int32_t n, int32_t nch,
		tms_vldelta_info_t *vld) {
	...
	/*Bug? In Mobita vldelta_info has different structure (unknown) and last samplediv is filled with checksum
	 * which causes error later*/
	if (vld->SampDiv[nch-1]>10) vld->SampDiv[nch-1]=0;
...
int32_t tms_prt_vldelta_info(FILE *fp, tms_vldelta_info_t *vld, int nr, int hdr) {
	...
	nc += fprintf(fp, " %6d %6d %6d %6d", nr, vld->Config, vld->Length,
			vld->TransFreqDiv);
...
int32_t tms_prt_rtc(FILE *fp, tms_rtc_t *rtc, int nr, int hdr) {
	...
	nc += fprintf(fp, " %6d %02d%02d-%02d-%02d %02d:%02d:%02d %8d", nr,
			rtc->century, rtc->year, rtc->month, rtc->day, rtc->hours,
			rtc->minutes, rtc->seconds, utc);
	nc += fprintf(fp, " %6d %6d\n", nr - pnr, utc - putc);
...
tms_channel_data_t *tms_alloc_channel_data() {
	...
	chd = (tms_channel_data_t *) calloc(in_dev->NrOfChannels,
			sizeof(tms_channel_data_t));
...
int32_t tms_init(char *fname, int32_t sample_rate_div) {
	...
	printf("Opened bluetooth socket %d\n", fd);
	...
	if (state == 5) {
		...
		if (tms_chk_msg(resp, br) != 0) {
			...
			case TMSCHANNELDATA:
				...
				/* check zaag !!! number 13 !!! */
				zaag = channel[13].data[0].isample;
				if ((zaag - pzaag + 64) % 64 != 2) {
					fprintf(stderr, "# Zaag: %d PZaag: %d\n", zaag, pzaag);
					/* correct data packet counter with saw jump */
					/* !!! 5 bits for saw is too small -> firmware fix in Mobi-8 */
					dpc += ((zaag - pzaag + 62) % 64) / 2;
					/* saw error */
					zerr = 1;
					tzerr++;
				}
