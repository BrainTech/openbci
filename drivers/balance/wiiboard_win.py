...
class Wiiboard(object):
	...
	self.board.write(self.zero_pad([0x16, 0x04, 0xa4, 0x00, 0xf0, 0x01, 0x55], BUF_OUT))
	self.board.write(self.zero_pad([0x16, 0x04, 0xa4, 0x00, 0xf0, 0x01, 0x55], BUF_OUT))
	#print self.board.read(BUF_IN)
	time.sleep(0.5) 
	self.board.write(self.zero_pad([0x16, 0x04, 0xa4, 0x00, 0xfb, 0x01, 0x00], BUF_OUT))
	#print self.board.read(BUF_IN)
	time.sleep(0.5)
	self.board.write(self.zero_pad([0x12, 0x04, 0x32], BUF_OUT))
	time.sleep(0.5)
	#print self.board.read(BUF_IN)
